...
protected:
	...
	VOID Run(Handle<_sender_t> Sender, _args_t... Arguments)
		{
		assert(Task::IsMainTask());
		auto current_ptr=&m_Handler;
		while(*current_ptr)
...
public:
	...
	template <class _owner_t> inline VOID Add(Handle<_owner_t> const& Owner, VOID (_owner_t::*Procedure)())
		{
		auto handler=new EventMemberFunction<_sender_t, _owner_t, _args_t...>(Owner, Procedure);
		_base_t::AddHandler(handler);
		}
	...
	template <class _owner_t> inline VOID Add(Handle<_owner_t> const& Owner, VOID (_owner_t::*Procedure)(_args_t...))
		{
		auto handler=new EventMemberFunctionWithArgs<_sender_t, _owner_t, _args_t...>(Owner, Procedure);
		_base_t::AddHandler(handler);
		}
	...
	template <class _owner_t> inline VOID Add(Handle<_owner_t> const& Owner, VOID (_owner_t::*Procedure)(_sender_t*, _args_t...))
		{
		auto handler=new EventMemberFunctionWithSender<_sender_t, _owner_t, _args_t...>(Owner, Procedure);
		_base_t::AddHandler(handler);
		}
