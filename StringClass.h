...
public:
	...
	// Access
	inline LPCTSTR Begin()const noexcept
		{
		return m_Buffer;
		}
	inline BOOL Contains(LPCSTR Value)noexcept
		{
		return StringHelper::FindString(m_Buffer, Value);
		}
	inline BOOL Contains(LPCSTR Value, CompareMode Mode)noexcept
		{
		return StringHelper::FindString(m_Buffer, Value, (UINT*)nullptr, Mode);
		}
	inline BOOL Contains(LPCWSTR Value)noexcept
		{
		return StringHelper::FindString(m_Buffer, Value);
		}
	inline BOOL Contains(LPCWSTR Value, CompareMode Mode)noexcept
		{
		return StringHelper::FindString(m_Buffer, Value, (UINT*)nullptr, Mode);
		}
	inline UINT Copy(LPSTR Buffer, UINT Size)noexcept
		{
		return StringHelper::Copy(Buffer, Size, m_Buffer);
		}
	inline UINT Copy(LPWSTR Buffer, UINT Size)noexcept
		{
		return StringHelper::Copy(Buffer, Size, m_Buffer);
		}
	...
	inline INT ToInt()
		{
		return StringHelper::ToInt(m_Buffer);
		}
	inline BOOL ToInt(INT* Value)noexcept
		{
		return StringHelper::ToInt(m_Buffer, Value);
		}
	inline INT64 ToInt64()
		{
		return StringHelper::ToInt64(m_Buffer);
		}
	inline BOOL ToInt64(INT64* Value)noexcept
		{
	...
	inline BOOL ToUInt(UINT* Value)noexcept
		{
		return StringHelper::ToUInt(m_Buffer, Value);
		}
	inline UINT64 ToUInt64()
		{
		return StringHelper::ToUInt64(m_Buffer);
		}
	inline BOOL ToUInt64(UINT64* Value)noexcept
		{
		return StringHelper::ToUInt64(m_Buffer, Value);
		}
...
public:
	...
	inline Handle(nullptr_t)noexcept:
		m_Object(nullptr)
		{}
	inline Handle(String* Copy)noexcept
		{
		Handle<Object>::Create(&m_Object, Copy);
		}
	inline Handle(Handle const& Copy)noexcept
		{
		Handle<Object>::Create(&m_Object, Copy.m_Object);
		}
	...
	inline Handle(LPCSTR Value):
		m_Object(nullptr)
		{
		operator=(Value);
		}
	inline Handle(LPCWSTR Value):
		m_Object(nullptr)
		{
		operator=(Value);
		}
	inline ~Handle()noexcept
		{
		Handle<Object>::Clear(&m_Object);
		}

	// Access
	inline operator bool()const noexcept
		{
		return m_Object&&m_Object->HasValue();
		}
	inline operator String*()const noexcept
		{
		return m_Object;
		}
	inline String* operator->()const noexcept
		{
		return m_Object;
		}
	inline SIZE_T WriteToStream(OutputStream* Stream)
		{
		return String::WriteToStream(m_Object, Stream);
		}
	...
	inline bool operator==(Handle<String> const& Value)const noexcept
		{
		return StringHelper::Compare(m_Object, Value)==0;
		}
	inline bool operator==(LPCSTR Value)const noexcept
		{
		return StringHelper::Compare(m_Object, Value)==0;
		}
	inline bool operator==(LPCWSTR Value)const noexcept
		{
		return StringHelper::Compare(m_Object, Value)==0;
		}
	inline bool operator!=(nullptr_t)const noexcept
	...
	inline Handle& operator=(String* Value)noexcept
		{
		Handle<Object>::Set(&m_Object, Value);
		return *this;
		}
	inline Handle& operator=(Handle<String> const& Value)noexcept
		{
		return operator=(Value.m_Object);
		}
	inline Handle& operator=(LPCSTR Value)
		{
		return operator=(String::Create(Value));
		}
